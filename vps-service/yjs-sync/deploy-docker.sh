...
ssh ${VPS_USER}@${VPS_HOST} << 'REMOTE'
# Configure Docker to use IPv4 only and proper DNS (VPS doesn't have IPv6)
if ! grep -q "dns" /etc/docker/daemon.json 2>/dev/null; then
  echo "Configuring Docker for IPv4 only and proper DNS..."
  sudo mkdir -p /etc/docker
  echo '{"ipv6": false,"fixed-cidr-v6": "", "dns": ["1.1.1.1", "8.8.8.8"]}' | sudo tee /etc/docker/daemon.json
