...
```bash
tailscale status
# You should see the VPS (REDACTED_IP) in the peer list
```
...
```bash
ssh root@REDACTED_IP "cat /root/yjs-sync/.env | grep TRANSCRIPT_API_KEY"
```
...
[mac-mini-1] VPS URL: http://REDACTED_IP:8766
...
- Test connection: `curl http://REDACTED_IP:8766/health`
