...
def get_video_info_ytdlp(video_id, use_proxy=True):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if use_proxy and ('data blocks' in error or 'proxy' in error.lower() or 'connection' in error.lower()):
                print(f"[{video_id}] Proxy failed for info, retrying without proxy...", file=sys.stderr)
...
def download_subtitles_ytdlp(video_id, lang='en', use_proxy=True):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if error_output:
                # If proxy fails, retry without proxy
                if use_proxy and ('data blocks' in error_output or 'proxy' in error_output.lower() or 'connection' in error_output.lower()):
                    print(f"[{video_id}] Proxy failed, retrying without proxy...", file=sys.stderr)
    ...
    while i < len(lines):
        ...
        if '-->' in line:
            try:
                ...
                end_str = parts[1].strip().split()[0]  # Take only first part (timestamp) from end
...
# =============================================================================
# WORKER QUEUE (for home workers)
# =============================================================================
# Pending video IDs that workers should process
...
def is_worker_available():
    ...
    global active_workers, pending_worker_queue
    cutoff = datetime.now() - timedelta(seconds=WORKER_TIMEOUT_SECONDS)
    # Remove stale workers - use dict comprehension and update global
    active_workers = {
        wid: t for wid, t in active_workers.items()
        if t > cutoff
    }
    print(f"[DEBUG] Active workers: {list(active_workers.keys())}, Pending queue: {pending_worker_queue}")
...
def worker_poll():
    ...
    print(f"[DEBUG] Worker {worker_id} polled, active workers: {list(active_workers.keys())}")

    # Return up to 5 pending videos
...
def worker_upload():
    ...
    # Build result matching fetch_transcript output format
    result = {
    ...
    print(f"[{video_id}] Received transcript from worker {worker_id}: {len(segments)} segments")
...
def get_transcript(video_id):
    ...
    if is_worker_available():
        # Queue to worker and return pending response
        if video_id not in pending_worker_queue:
    ...
    try:
        ...
        import traceback
        print(f"ERROR in fetch_transcript: {e}", file=sys.stderr)
